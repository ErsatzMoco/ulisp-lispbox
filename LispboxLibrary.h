...
(defun se:save ()
	...
	(let ((fname (se:input "SAVE file name (UPPERCASE only): " se:filename 8 t)) (suffix (se:input "Suffix: ." se:suffix 3 t)) (overwrite t) (fullname ""))
...
(defun se:quicksave ()
	(when se:editable
		(keyboard-flush)
		(sd-file-remove "BACKUP.CL")
		(with-sd-card (strm "BACKUP.CL" 2)
			(dolist (line se:buffer)
				(princ line strm)
				(princ (code-char 10) strm)
			)
...
(defun se:save-buffer ()
	...
	(setq se:buffer ())
	(tft1-set-cursor 0 0)
	(tft1-set-text-color se:code_col se:line_col)
	(if se:help-active
		(tft1-write-text "F1/F2 chk () | F3 help | F4 BACK                            | F9 del | F10 save                     ")
		(tft1-write-text "F1/F2 chk () | F4 BACK                                      | F9 del | F10 save                     ")
	)
	(when se:match
		(tft1-set-cursor 0 0)
		(tft1-set-text-color se:bg_col se:emph_col)
...
(defun se:viewsym ()
	...
	(se:hide-cursor)
	(if se:editable
		(let ((symname (se:input "SYMBOL: " nil 60)))
...
(defun se:loadview ()
	...
	(se:hide-cursor)
	(if se:editable
		(let ((fname (se:input "VIEW file (UPPERCASE only): " nil 8 t)) (suffix (se:input "Suffix: ." "CL" 3 t)) (line ""))
			(unless (or (< (length fname) 1) (< (length suffix) 1) (not (sd-file-exists (concatenate 'string fname "." suffix))))
				(setf se:editable nil)
				(se:save-buffer)
				(with-sd-card (strm (concatenate 'string fname "." suffix) 0)
				...
				(when lastkey
					(when (or newkey krepeat)
						(case lastkey
							((1 210) (se:linestart))
							((5 213) (se:lineend))
							((3 17) (when (se:alert "Exit") (se:cleanup) (setf se:exit t)) (keyboard-flush) (setf lastkey nil))
							((24 14 2) (se:flush-buffer) (setf lastkey nil))
							((11 12 253) (se:flush-line) (setf lastkey nil))
							(94 (se:docstart))
							(211 (se:prevpage))
							(214 (se:nextpage))
							...
							(197 (se:viewsym) (setf lastkey nil))
							(198 (se:run) (setf lastkey nil))
							(200 (se:loadview) (setf lastkey nil))
							(201 (se:quicksave) (setf lastkey nil))
							...
							((240 241 242 243 244 245 246 247 248 249 250) (se:snippet lastkey) (setf lastkey nil))
